    ...
    <>
      <Navbar onChange={(sfs) => {
        ...
        for(let i = 0;i < sfs.length;++i){
          const rv = sfs[i].rv
          if(rv != null){
            setCast(cast.filter((c) => c[sfs[i].filter.field as keyof typeof c] >= sfs[i].lv && c[sfs[i].filter.field as keyof typeof c] <= rv));
          }else{
            setCast(cast.filter((c) => c[sfs[i].filter.field as keyof typeof c] == sfs[i].lv));
          }
        }
      }}/>
      <ReactTabulator
        columns={[
          ...
          { title: "gender", field: "gender" },
          {
            ...
            formatter: function(cell: Tabulator.CellComponent){
              return DateTime.fromMillis(cell.getValue()).toFormat("yyyy/MM/dd");
            }
          },
        ]}
        data={cast}
